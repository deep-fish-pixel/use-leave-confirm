...
export const login = function (params: { username: string; password: string }) {
  return client.post<AxiosResponse<HttpResponse<{ account: string }>>>('/api/user/login', params);
};

export const getUser = function (userId: string) {
  return client.get<AxiosResponse<HttpResponse<UserModel>>>(`/api/user/${userId}`);
};
